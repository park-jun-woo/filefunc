...
| `//ff:calls` | 호출하는 함수 목록 | 필수 | 정적 관계, 코드 변경 시만 업데이트 |
| `//ff:uses` | 사용하는 타입 목록 | 필수 | 정적 관계, 코드 변경 시만 업데이트 |
...
```bash
filefunc validate ./internal/           # 코드 구조 룰 검증
filefunc annotate ./internal/           # LLM 자동 어노테이션 생성
