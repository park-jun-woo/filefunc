...
func TestFilterByFeature(t *testing.T) {
	files := []model.SourceFile{
		&model.GoFile{Package: "v", Funcs: []string{"RuleF1"}, Annotation: &model.Annotation{Func: map[string]string{"feature": "validate"}}},
		&model.GoFile{Package: "c", Funcs: []string{"Build"}, Annotation: &model.Annotation{Func: map[string]string{"feature": "chain"}}},
	}
