...
//ff:what Python 프로젝트 루트에서 호출 그래프를 구성
package cli
...
// BuildGraphPython builds a call graph from a Python project at root.
func BuildGraphPython(root string) (*chain.CallGraph, []model.SourceFile, error) {
	ignorePatterns := walk.ParseFFIgnore(filepath.Join(root, ".ffignore"))
	files, err := LoadPythonFiles(root, ignorePatterns)
	if err != nil {
		return nil, nil, err
	}
