...
//ff:what TypeScript용 toulmin defeats graph — TypeScript 파일의 F/Q/A 룰과 예외 관계를 선언
package validate

import "github.com/park-jun-woo/toulmin/pkg/toulmin"

// TypeScriptValidateGraph declares TypeScript validation rules and their defeat relationships.
var TypeScriptValidateGraph = newTypeScriptValidateGraph()

func newTypeScriptValidateGraph() *toulmin.Graph {
	g := toulmin.NewGraph("validate-typescript")
	...
	g.Warrant(CheckFuncLinesTypeScript, nil, 1.0)
	g.Warrant(CheckControlBodyTypeScript, nil, 1.0)
	...
	// ── 어노테이션: 위치, 해시, 차원 ──
	g.Warrant(AnnotationAtTopTypeScript, nil, 1.0)
	g.Warrant(CheckedHashMatchTypeScript, nil, 1.0)
	wA16 := g.Warrant(ValidDimension, nil, 1.0)

	// ── 어노테이션: 제어 구조 일치 (TypeScript 전용) ──
	wA10 := g.Warrant(ControlMatchTypeScript, &ControlMatchBacking{Control: "selection", MustHave: "switch", Rule: "A10",
		Message: "control=selection but no switch found at depth 1"}, 1.0)
	wA11 := g.Warrant(ControlMatchTypeScript, &ControlMatchBacking{Control: "iteration", MustHave: "loop", Rule: "A11",
		Message: "control=iteration but no loop found at depth 1"}, 1.0)
	wA12 := g.Warrant(ControlMatchTypeScript, &ControlMatchBacking{Control: "sequence", MustNotHave: "switch|loop", Rule: "A12",
		Message: "control=sequence but %s found at depth 1; add control=%s or extract to separate func"}, 1.0)
	wA13 := g.Warrant(ControlMatchTypeScript, &ControlMatchBacking{Control: "selection", MustNotHave: "loop", Rule: "A13",
		Message: "control=selection but loop found at depth 1; extract loop to separate func"}, 1.0)
	wA14 := g.Warrant(ControlMatchTypeScript, &ControlMatchBacking{Control: "iteration", MustNotHave: "switch", Rule: "A14",
